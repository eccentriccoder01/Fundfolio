...
const AllocationForm = () => {
    ...
    const submitEvent = async () => {
        ...
        try {
            ...
            dispatch({
                type: 'ADD_NOTIFICATION',
                payload: { message: 'An error occurred while processing your request', type: 'error' }
            });
        } finally {
    ...
    const currentAllocation = expenses.find(exp => exp.name === name)?.cost || 0;
            ...
            <div className="form-grid">
                ...
                <div className="form-group">
                    <button 
                        ...
                        style={{
                            opacity: (isSubmitting || !name || !cost) ? 0.6 : 1,
                            cursor: (isSubmitting || !name || !cost) ? 'not-allowed' : 'pointer',
                            display: 'flex',
                            alignItems: 'center',
                        ...
                        {[100, 250, 500, 1000].map(amount => (
                            <button
                                ...
                                style={{
                                    ...
                                    border: '1px solid var(--border-color)',
                                    background: cost === amount.toString() ? 'var(--primary-color)' : 'var(--bg-primary)',
                                    color: cost === amount.toString() ? 'white' : 'var(--text-primary)',
                                    borderRadius: '8px',
                                    fontSize: '0.75rem',
                                    cursor: 'pointer',
